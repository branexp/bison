...
def create_campaign(
    ctx: typer.Context,
    file: Path | None = typer.Option(None, "--file", help="Campaign workflow JSON file."),
    # Core
    name: str | None = typer.Option(None, "--name", help="Campaign name."),
    type: str = typer.Option("outbound", "--type", help="Campaign type (outbound|reply_followup)."),
    # Leads
    lead_list_id: int | None = typer.Option(None, "--lead-list-id", help="Import leads from existing list id."),
    lead_id: list[int] | None = typer.Option(None, "--lead-id", help="Import leads by lead id (repeatable)."),
    ...
    plain_text: bool | None = typer.Option(None, "--plain-text/--html", help="Send plain text only."),
    open_tracking: bool | None = typer.Option(None, "--open-tracking/--no-open-tracking"),
    reputation_building: bool | None = typer.Option(None, "--reputation-building/--no-reputation-building"),
    can_unsubscribe: bool | None = typer.Option(None, "--can-unsubscribe/--no-can-unsubscribe"),
    ...
    sequence_file: Path | None = typer.Option(
        None, "--sequence-file", help="JSON file containing {title, sequence_steps}."
    ),
    # Config overrides
    ...
    if file is not None:
        ...
        if any(v is not None for v in (schedule_timezone, schedule_start, schedule_end)):
            if not (schedule_timezone and schedule_start and schedule_end):
                typer.echo(
                    "If setting schedule, provide --schedule-timezone, --schedule-start, and --schedule-end",
                    err=True,
                )
                raise typer.Exit(code=2)
    ...
    try:
        ...
        if spec.sequence is not None:
            ...
            steps = [s.model_dump(exclude_none=True) for s in spec.sequence.sequence_steps]
    ...
    if debug:
        typer.echo(
            f"debug: method={dbg_create.method} status={dbg_create.status_code} url={dbg_create.url}",
            err=True,
        )
        typer.echo(
